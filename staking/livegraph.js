...
function liveGraph(targetDiv, data, parameters) {
  ...
  this.update = function (data, xVar, yVar) {
    ...
    if (yScale.ticks().length > 150) {
      this.svg.select(".y.axis").call(yAxis).selectAll("text").remove();
    }

    var circle = this.svg.selectAll("circle").data(data, function key(d) {
      return d[xVar];
    });
    ...
    circle
      ...
      .duration(1000)
      .each("start", function () {
        d3.select(this).attr("class", "datadotsmall").attr("r", 2);
      })
      .each("end", function () {
        d3.select(this)
          .transition()
