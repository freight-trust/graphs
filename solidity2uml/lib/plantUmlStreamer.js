...
exports.streamTransferPuml = (txHash, transfers, contracts, options = {}) => {
  const pumlStream = new stream_1.Readable({
    read() {},
  });
  pumlStream.push(`@startuml\ntitle ${txHash}\n`);
  exports.writeParticipants(pumlStream, contracts);
  exports.writeTransfers(pumlStream, transfers, options);
  pumlStream.push("\n@endumls");
  pumlStream.push(null);
  return pumlStream;
};
exports.streamTxPlantUml = (transactions, contracts, options = {}) => {
  const pumlStream = new stream_1.Readable({
    read() {},
  });
  if (Array.isArray(transactions)) {
    exports.streamMultiTxsPuml(pumlStream, transactions, contracts, options);
  } else {
    exports.streamSingleTxPuml(pumlStream, transactions, contracts, options);
  }
  return pumlStream;
};
exports.streamMultiTxsPuml = (
  pumlStream,
  transactions,
  contracts,
  options = {}
  ...
  for (const transaction of transactions) {
    ...
    exports.writeParticipants(pumlStream, contracts);
    exports.writeMessages(pumlStream, transaction.messages, options);
    pumlStream.push("end");
  }
  pumlStream.push("\n@endumls");
  pumlStream.push(null);
...
exports.writeParticipants = (plantUmlStream, contracts) => {
  ...
  for (const [address, contract] of Object.entries(contracts)) {
    ...
    plantUmlStream.push(
      `participant "${exports.shortAddress(
        address
      )}" as ${exports.participantId(address)} ${name}\n`
    );
  }
};
  ...
  for (const message of messages) {
    debug(
      `id ${message.id}, parent ${
        message.parentId
      }, from ${exports.shortAddress(message.from)}, to ${exports.shortAddress(
        message.to
      )}, ${
        (_a =
        ...
        (_b =
          ...
          ? void 0
          : _b.funcSelector
      }, type ${message.type}, delegated call ${
        (_c = message.delegatedCall) === null || _c === void 0 ? void 0 : _c.id
      } last ${
        (_d = message.delegatedCall) === null || _d === void 0
          ? void 0
          : _d.last
      }`
    );
    ...
    if (
      (_e =
        previousMessage === null || previousMessage === void 0
          ? void 0
          : previousMessage.delegatedCall) === null || _e === void 0
        ? void 0
        : _e.last
    ) {
    ...
    if (
      message.type === transaction_1.MessageType.Call ||
      message.type === transaction_1.MessageType.Create ||
      message.type === transaction_1.MessageType.Delegatecall
    ) {
      // output call message
      plantUmlStream.push(
        `${exports.participantId(message.from)} ${genArrow(
          message
        )} ${exports.participantId(message.to)}: ${genFunctionText(
          message,
          options.params
        )}${genGasUsage(message, options.gas)}${genEtherValue(
          message,
          options.ether
        )}\n`
      );
      if (message.type === transaction_1.MessageType.Delegatecall) {
        plantUmlStream.push(
          `activate ${exports.participantId(
            message.to
...
const genEndLifeline = (message) => {
  ...
  if (message.error) {
    plantUml += `note right of ${exports.participantId(message.to)}: ${
      message.error
    }\n`;
  }
  return plantUml;
...
const genArrow = (message) => {
  var _a;
  const arrowColor = isNaN(
    (_a = message.delegatedCall) === null || _a === void 0 ? void 0 : _a.id
  )
    ? ""
...
exports.writeTransfers = (plantUmlStream, transfers, options = {}) => {
  if (
    !(transfers === null || transfers === void 0 ? void 0 : transfers.length)
  ) {
    return;
  }
  plantUmlStream.push("\n");
  for (const transfer of transfers) {
    debug(
      `id ${transfer.id}, from ${exports.shortAddress(
        transfer.from
      )}, to ${exports.shortAddress(
        transfer.to
      )}, value ${transfer.value.toString()}`
    );
    const valueUnit = transfer.symbol ? transfer.symbol : "ETH";
    plantUmlStream.push(
      `${exports.participantId(transfer.from)} -> ${exports.participantId(
        transfer.to
