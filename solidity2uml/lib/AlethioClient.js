...
exports.getTransactionDetails = async (txHash, apiKey, network = "mainnet") => {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `no transaction attributes in Alethio response: ${
          response === null || response === void 0 ? void 0 : response.data
        }`
      );
    }
    if (
      ...
      throw new Error(
        `no transaction relationships in Alethio response: ${
          response === null || response === void 0 ? void 0 : response.data
        }`
      );
    }
...
exports.getContractMessages = async (txHash, apiKey, network = "mainnet") => {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `no contract messages in Alethio response ${
          response === null || response === void 0 ? void 0 : response.data
        }`
      );
    }
    for (const contractMessage of response.data.data) {
      const parentId =
        (_e =
          (_d =
            (_c =
              (_b = contractMessage.relationships.parentContractMessage) ===
                null || _b === void 0
                ? void 0
    ...
    if (
      (_h =
        (_g =
          (_f = response.data) === null || _f === void 0 ? void 0 : _f.meta) ===
          null || _g === void 0
          ? void 0
          : _g.page) === null || _h === void 0
        ? void 0
        : _h.hasNext
    ) {
...
const identifyDelegateCalls = (messages) => {
  try {
    ...
    messages.forEach((message, i) => {
      if (!isNaN(message.parentId)) {
        ...
        if (messages[i].from !== messages[message.parentId - 1].to) {
          messages[message.parentId - 1].type =
            transaction_1.MessageType.Delegatecall;
    ...
    const delegateCallIds = Object.keys(delegateCounts).map((id) =>
      parseInt(id)
    );
...
const convertDecimal = (value, decimals = 18) => {
  const valueBN = new bignumber_js_1.default(value.toString());
  return valueBN.div(new bignumber_js_1.default(10).pow(decimals));
};
exports.getToken = async (contractAddress, apiKey, network = "mainnet") => {
  var _a, _b, _c;
  if (
    !(contractAddress === null || contractAddress === void 0
      ? void 0
      : contractAddress.match(regEx_1.ethereumAddress))
  ) {
    throw new TypeError(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `no token attributes in Alethio response: ${
          response === null || response === void 0 ? void 0 : response.data
        }`
      );
    }
    ...
    if (
      ((_c = err === null || err === void 0 ? void 0 : err.response) === null ||
      _c === void 0
        ? void 0
        : _c.status) === 404
    ) {
...
exports.getTokenTransfers = async (txHash, apiKey, network = "mainnet") => {
  var _a, _b, _c;
  if (
    !(txHash === null || txHash === void 0
      ? void 0
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `no token transfers in Alethio response ${
          response === null || response === void 0 ? void 0 : response.data
        }`
      );
    }
    for (const transfer of response.data.data) {
      let id = 1;
      let type = "TokenTransfer";
      if (
        (_b = transfer.relationships.contractMessage.data) === null ||
        _b === void 0
          ? void 0
          : _b.id
      ) {
        ...
        (_c = transfer.attributes) === null || _c === void 0
          ? void 0
          : _c.globalRank[2]
      ) {
...
exports.getEtherTransfers = async (txHash, apiKey, network = "mainnet") => {
  const contractMessages = await exports.getContractMessages(
    txHash,
    apiKey,
    network
  );
  const etherMessages = contractMessages.filter((message) => {
    var _a;
    return (_a = message.value) === null || _a === void 0 ? void 0 : _a.gt(0);
  });
  debug(
    `Got ${etherMessages.length} Ether transfers from ${contractMessages.length} contract messages`
  );
  const [, firstMessage] = await exports.getTransactionDetails(
    txHash,
    apiKey,
    network
  );
...
exports.getTransfers = async (txHash, apiKey, network = "mainnet") => {
  const etherTransfers = await exports.getEtherTransfers(
    txHash,
    apiKey,
    network
  );
  const tokenTransfers = await exports.getTokenTransfers(
    txHash,
    apiKey,
    network
  );
