...
const tx2uml = async () => {
  ...
  if (program.transfers) {
    ...
    const transfers = await AlethioClient_1.getTransfers(
      txHash,
    ...
    const contracts = await transaction_1.getContractsFromAddresses(
      participants,
      options
    );
    pumlStream = await plantUmlStreamer_1.streamTransferPuml(
      txHash,
      transfers,
      contracts,
      options
    ...
    let transactions;
    if (
      (_a = program.args[0]) === null || _a === void 0
        ? void 0
        : _a.match(regEx_1.transactionHash)
    ) {
      transactions = await transaction_1.getTransaction(
        program.args[0],
        options
      );
    } else {
      try {
        const txHashes =
          (_b = program.args[0]) === null || _b === void 0
            ? void 0
            : _b.split(",");
  ...
  if (!filename) {
    filename = (
      (_c = program.args[0]) === null || _c === void 0
        ? void 0
        : _c.match(regEx_1.transactionHash)
    )
      ? program.args[0]
      : "output";
  }
